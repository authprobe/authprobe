...
func TestVersionFlag(t *testing.T) {
	...
	if !strings.Contains(output, "authprobe") {
		t.Fatalf("expected output to contain authprobe, got %q", output)
	}
	if !strings.Contains(output, "dev") {
		t.Fatalf("expected output to contain default version, got %q", output)
	}
}
