...
| 3    | PRM Discovery        | Fetch Protected Resource Metadata (path-suffix/resource_metadata preferred; root for compatibility) | `200 OK`, JSON with `resource` + `authorization_servers`                 | [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728), [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986)        |
...
AuthProbe attempts authorization server discovery in a bounded fallback sequence: RFC 8414 insertion form first, then OIDC discovery insertion form, and finally the OIDC append form used by some issuers (e.g., Entra). If any candidate yields valid metadata, Step 4 passes.
